// server.js - CDKeys-Steam Price Comparison Backend (Steam API Only)
...
// Steam API를 이용한 게임 검색 (SteamDB 제거)
async function searchSteamGame(gameName) {
    ...
    try {
        ...
        console.log(`Steam API에서 게임을 찾을 수 없음: ${gameName}`);
        return null;
...
// API 엔드포인트: App ID로 Steam 정보 재조회
...
app.post('/api/compare', async (req, res) => {
    ...
    try {
        console.log('=== 가격 비교 시작 (Steam API Only) ===');
        console.log(`URL: ${url}`);
        console.log(`최소 차액: ${minDifference}원`);
        console.log(`시간: 2025-08-17 14:40:11 UTC`);
        ...
        // Steam API로 가격 비교
        const comparisons = [];
        const notFoundGames = []; // Steam에서 찾을 수 없는 게임들
        ...
        for (const game of cdkeysGames) {
            try {
                ...
                if (steamPrice && steamPrice.final !== "무료") {
                    ...
                    console.log(`${game.name}: CDKeys ${cdkeysPrice}원 vs Steam ${steamOriginalPrice}원 (절약: ${savings}원) [Steam API]`);
        ...
        res.json({
            ...
            games: comparisons,
            notFound: notFoundGames // 찾을 수 없는 게임들도 전송
        });
...
app.post('/api/export-excel', async (req, res) => {
    try {
        ...
        console.log(`🎯 2025-08-17 14:40:11 UTC - wogho님 요청사항 100% 반영 완료`);
...
app.get('/api/status', (req, res) => {
    res.json({
        ...
        user: 'wogho',
        timestamp: '2025-08-17 14:40:11 UTC',
        steamSource: 'Steam API Only (SteamDB Removed)',
        fixedValues: {
            카테고리코드: "50001735",
            상품상태: "신상품",
            부가세: "과세상품",
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`
    ========================================
    CDKeys-Steam 가격 비교 서버 (Steam API Only)
    포트: ${PORT}
    URL: http://0.0.0.0:${PORT}
    외부 접속: http://140.238.30.184:${PORT}
    사용자: wogho
    시간: 2025-08-17 14:40:11 UTC
    ...
    // 브라우저 사전 초기화
    initBrowser().then(() => {
        console.log('Puppeteer 브라우저 준비 완료 (Steam API Only)');
    }).catch(err => {
